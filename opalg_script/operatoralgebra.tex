...
The set of all linear bounded operators $\algebra{L}(H) = \algebra{B}(H)$ on a given Banach space $H$ is a (Banach) algebra kkwith $S \cdot T = S \circ T$.
...
\begin{theorem}[Adjointing units - unitization of algebras]
	...
	Define
	\begin{align*}
		(a + \lambda 1) (b + \lambda' 1) & = (ab + \lambda' a + \lambda ab + b) + \lambda \cdot \lambda'\text{.}
	\end{align*}
	...
	If $\algebra{A}$ is Banach and closed, then so is $\algebra{A}$.
...
\begin{theorem}~
	\begin{enumerate}
		...
		\item If $a,b \in \algebra{A}$ then $e^{a + b} = e^a \cdot e^b$.
	\end{enumerate}
\end{theorem}
...
We have for $\algebra{A}$ a Banach algebra. We can add a unit to receive $\tilde{\algebra{A}}$, which i Banach algebra.

Observe: If $\chi \in \Omega(\algebra{A})$, then there exists a unique $\tilde{X} \in \Omega(\tilde{\algebra{A}})$ via $\tilde{X}(a + \lambda \cdot) = \chi(a) + \lambda$. Thus $\|\chi\| \leq \|\tilde{X}\| = 1$ (Note that it may still be greater than $1$. See exercises 2023-05-09).
...
\begin{proposition}
	$\Omega(\algebra{A})$ is a locally compact Hausdorff space. If $\algebra{A}$ is untrue, the n$\Omega(\algebra{(A)})$ is compact.
\end{proposition}
...
\begin{equation*}
	\Omega(\tilde{\algebra{A}}) \to \Omega(\algebra{A}) \sqcup \{ \chi_\infty\}, \phi \mapsto \left\{  \begin{matrix}
		\phi|_\algebra{A} & \phi|_{\algebra{A}} = 0\\
		\chi_\infty & \phi_|\algebra{A} = 0
...
\begin{theorem}
	...
	\begin{itemize}
		\item $\{ \chi(a) \mid \chi \in \Omega(\algebra{A})\} = \sigma$ in case $\algebra{A}$ is unital.
...
\begin{proof}
	\begin{itemize}
		\item $\algebra{A}$ is unital, $a \in \algebra{A}$. $\chi(a = \chi(a) \cdot 1) = 0$, so $\chi(a - \chi(a) \cdot 1 ) = 0$, so $\chi(a) \in \sigma(a)$.
		
		Then $\{ \chi(a) \mid x \in \Omega(a) \}  \subseteq \sigma(a)$. Now if $\lambda \in \sigma(a)$, consider $\ring{I} \coloneq (a = \lambda 1) \algebra{A} \triangleleft \algebra{A}$ if $\algebra{A}$ is commutative.

		By Zorns Lemma, we get $I \subseteq J \triangleleft \algebra{A}$. And thus $J = \ker(\chi)$, and some $\chi \in \Omega(\algebra{A})$. Thus we have $a - \lambda \cdot 1 \in \ring{I} \subseteq J = \ker(\chi)$ so $\chi(a) = \lambda$.
		...
		\begin{align*}
			\sigma_\algebra{A} = \sigma_{\tilde{\algebra{A}}} = \{\chi(a) \mid \chi \in \Omega(\tilde{\algebra{A}})\} = \{\chi(a) \mid \chi \in \Omega(\algebra{A}) \} \cup \{0\}
		\end{align*}
	\end{itemize}
\end{proof}
