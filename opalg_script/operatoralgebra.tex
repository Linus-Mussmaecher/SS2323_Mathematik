...
\begin{example}~
	\begin{enumerate}
		...
		\begin{equation*}
			\check{f} \coloneq \int_{\mathds{S}^1} f(z) \overline{z}^n dz = \frac{1}{2 \pi} \int_{0}^{1} f(\exp(2 \pi i t)) \exp(- 2 \pi i t n) dt 
		\end{equation*}
		You can now check that $(\hat{a})^{\check{~}}(n) = a_n$.
		In particular, $(\hat{\delta_m})^{\check{~}}(n) = \delta_{m,n}$ and therefore $(z^m)^{\check{~}}(n) = \delta_{m,n}$ so $\int_{\mathds{S}^1} z^{m-n} dz = \delta_{m,n}$?

		$\Gamma$ is not surjective.

		$\Gamma$ is not isometric.
	\end{enumerate}
\end{example}
