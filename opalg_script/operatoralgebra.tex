...
\begin{proof}
	...
	%$\|\phi(a)\|_\algebra{B} = \pi(\phi(a)) \leq \pi(a) \leq \|a\|_\algebra{A}$. %% WRONG => needs self-adjoint
	\begin{align*}
...
\begin{proof}[Gelfand]~
	\begin{itemize}
		...
		\end{itemize}
		The result $B = C_0(\Omega(\algebra{A}))$ follows from the Stone-Weierstraß-theorem:
		\begin{quote}
...
\begin{remark}
	...
	If we now start with $A = C_0(X)$, then $\Omega(C_0(X)) \simeq X$.
	So there is a bijection between locally compact Hausdorff spaces and commutative $C^*$-algebras.
	We prove this with a very canonical mapping:
	\begin{equation*}
		\Phi: X \to \Omega(C_0(X)), x \mapsto (\mathrm{ev}_x: C_0(X) \to \mathds{C}, f \mapsto f(x))\text{.}
	\end{equation*}
	The \textbf{continuity} is easy to see:
	Take $x_i \to x$ in $X$, then $\mathrm{ev}_{x_i} \to \mathrm{ev}_x$ in the weak $^*$-topology.
	Then, for all $f \in C_0(X)$ we have $\mathrm{ev}_{x_i}(f) \to \mathrm{ev}_x(f)$ because $f$ is continuous and therefore $f(x_i) \to f(x)$.

	The \textbf{injectivity} of $f$ is also easy to see.
	$x \mapsto \mathrm{ev}_x$ is injective because if $x_1 \neq x_2$ there exists a function $f \in C_0(X)$ that separates them, but then $\mathrm{ev}_{x_1}(f) \neq \mathrm{ev}_{x_2}(f)$, so $\mathrm{ev}_{x_1} \neq \mathrm{ev}_{x_2}$.

	This map is \textbf{surjective} and \textbf{open}, but this is more difficult to prove.
\end{remark}
