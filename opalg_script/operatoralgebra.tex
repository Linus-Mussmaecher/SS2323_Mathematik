...
\begin{example}~
	\begin{enumerate}
		...
		\begin{equation*}
			I(X) = \{ f : X \to X \mid f \text{ is } a \text{partial bisjection}
		\end{equation*}
		i.e. $f: U \to V$ where $U,V \subseteq X$ (note that these may be any set, even the empty set and need not be open, as $X$ does not even have a topology).
...
\begin{proof}[Idea]
	Take $s \in S$ and $\closure{X} = S$ defines a partial bijektion.
	$f_s(x) = sx$. 
