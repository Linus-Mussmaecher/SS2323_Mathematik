...
\begin{exercise}[05-01]
	...
	% Equivalent characteristics of pure states: If $\algebra{B}$ is a $C^*$-algebra then $\omega \in S(\algebra{B})$ is pure iff for all $f \in \algebra{B}^*_+$ we have that $f \leq \omega$ (i.e. $\omega - f \geq 0$) already implies the existence of a $t \in [0,1]$ with $f = t \cdot \omega$. So there are no non-trivial things below pure states -- the only things below are multiples.

	% So we had $\phi = \alpha \omega_1 + (1 - \alpha) \omega_2$ as a state, then 
	% \begin{align*}
	% 	\alpha \omega_1 \leq \phi &\wedge (1 - \alpha) \omega_2 \leq \phi \\
	% 	\alpha \omega_1 = t \phi  &\wedge (1 - \alpha) \omega_2 = s \phi
	% \end{align*}
	% so
	% \begin{equation*}
	% 	\phi = t \phi + s \phi = (t + s) \phi
	% \end{equation*}
	% so $t + s = 1$. 
